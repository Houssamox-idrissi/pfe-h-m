        ...
        <div class="px-5 bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
            ...
            <form @submit.prevent="login" class="space-y-4 md:space-y-6">
              ...
              <br>
              <input type="password" v-model="password" placeholder="Password" class="px-5 bg-gray-50 border border-secondary text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required>
  ...
  export default {
    ...
    methods: {
      login() {
        ...
        .then(response => {
          const token = response.data.access_token; // Correctly access the token
          if (token) {
