<template>
  <main>
    <router-link to="/admin/brand/create" class="btn btn-secondary" style="font-family: 'Arial', sans-serif; font-size: 18px;">
    ...
    <table class="table border-collapse border rounded-lg overflow-hidden table-striped" >
      ...
      <tbody>
        <tr v-for="(brand, index) in brands" :key="index" class="text-center">
          <td class="mr-24 bt-8" style="font-family: 'Arial', sans-serif; font-size: 16px; font-weight: bold; color: #333;">{{ brand.name }}</td>

          <td>
            <img :src="'http://127.0.0.1:8000/storage' + brand.image.replace('public', '')" class="img-fluid"
              style="max-width: 100px;">
          </td>
          <td class="mr-24">
  <router-link :to="{ path: '/admin/brands/' + brand.id + '/edit' }">
